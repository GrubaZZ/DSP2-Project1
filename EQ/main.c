...
float alphaLP = 0.9243905;
float alphaHP = 0.0392901;
float alphaPeek = 0.9390625;
float betaPeek = 0.9844266 ;
float alphaPeek2 = 0.6007122;
float betaPeek2 = 0.7001308;

//alpha0 = 0.9243904917414228
//alpha1 = 0.9390625058850465
//alpha2 = 0.6007121634154783
//alpha3 = 0.03929010786156155
//beta1 = 0.9844265681253894
//beta2 = 0.7001308125473247
//0.1 -> jako siroko
//0.9 jako usko
...
{   
	...
	int k[4] = {MAXnum, MAXnum, MAXnum, MAXnum};
	int selected = 0;
    	...
    	aic3204_read_block(sampleBufferL, sampleBufferR);
				...
				switch (key) {
					...
					case SW2:
						...
						{
							k[selected]-=3277;
							if(k[selected] < 0) k[selected] = MAXnum;
							clearLCD();
    	    	...
    	    	for (i = 0; i < AUDIO_IO_SIZE; i++)
    	    	{
    	    		ShellBufferLP[i] = shelvingLP(dirakSample[i], coeffLP ,x_historyLP, y_historyLP, MAXnum4);
    	    	}
    	    	...
    	    	for (i = 0; i < AUDIO_IO_SIZE; i++)
				{
					ShellBufferHP[i] = shelvingHP(dirakSample[i], coeffHP ,x_historyHP, y_historyHP, MAXnum4);
				}
    	    	...
    	    	for (i = 0; i < AUDIO_IO_SIZE; i++)
				{
					ShellBufferPeek[i] = shelvingPeek(dirakSample[i], coeffPeek ,x_historyPeek, y_historyPeek, MAXnum4);
				}
    	    	...
    	    	calculatePeekCoeff(alphaPeek2, betaPeek2, coeffPeek2);
				for (i = 0; i < AUDIO_IO_SIZE; i ++)
				{
